...
{	gdprintf4("%s: dev=0x%lx(%s) depth=%d\n",
		 cname, (ulong)dev, dev->dname, dev->color_info.depth);
	gdprintf4("  source data=0x%lx x=%d raster=%u id=%lu colors=",
	...
	if ( scolors )
	  ...
	  dputs("none;\n");
	if ( textures )
	  gdprintf8("  textures=0x%lx size=%dx%d(%dx%d) raster=%u shift=%d(%d)",
	  ...
	  dputs("  textures=none");
	if ( tcolors )
	  gdprintf2(" colors=(%lu,%lu)\n", tcolors[0], tcolors[1]);
	else
	  dputs(" colors=none\n");
	gdprintf7("  rect=(%d,%d),(%d,%d) phase=(%d,%d) op=0x%x\n",
