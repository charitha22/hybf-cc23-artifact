...
{	int i, j;
	gdprintf3("[L]width=%d, height=%d, raster=%d\n",
		 width, height, raster);
	for ( i = 0; i < height; i++ )
  ...
  if ( cb_limit > cb_end ) cb_limit = cb_end
		...
		if ( gs_debug_c('L') )
		  { const char **sub = cmd_sub_op_names[op >> 4];
		    if ( sub )
		      gdprintf1("[L]%s:", sub[op & 0xf]);
		    else
		      gdprintf2("[L]%s %d:", cmd_op_names[op >> 4], op & 0xf);
		  }
#endif
		switch ( op >> 4 )
		   {
				...
				{ int n = *cbp++;
				  if ( ht_data_index < porder->num_levels )
					...
					(porder->levels + ht_data_index);
				      ...
				      if ( gs_debug_c('L') )
					{ int i;
					  gdprintf1(" levels[%u]", ht_data_index);
					  for ( i = 0; i < n; ++i )
					    gdprintf1(" %u",
					...
					(porder->bits +
					 (ht_data_index - porder->num_levels));
				      ...
				      if ( gs_debug_c('L') )
					{ int i;
					  gdprintf1(" bits[%u]", ht_data_index - porder->num_levels);
					  for ( i = 0; i < n; ++i )
					    { const gx_ht_bit *pb =
		...
		case cmd_op_misc2 >> 4:
			...
			case cmd_opv_set_dash:
				{ int nb = *cbp++;
				  ...
				  if ( gs_debug_c('L') )
				    { int i;
				      gdprintf4(" dot=%g(mode %d) adapt=%d offset=%g [",
					       dot_length,
					       (nb & 0x40) != 0,
					       (nb & 0x80) != 0, offset);
				      for ( i = 0; i < n; ++i )
					gdprintf1("%g ", dash_pattern[i]);
				      dputs("]\n");
				    }
#endif
				  cbp += n * sizeof(float);
...
bad_op:			lprintf5("Bad op %02x band y0 = %d file pos %ld buf pos %d/%d\n",
			   ...
			   {	const byte *pp;
				for ( pp = cbuf; pp < cb_end; pp += 10 )
				  { gdprintf1("%4d:", (int)(pp - cbuf));
				    gdprintf10(" %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x\n",
