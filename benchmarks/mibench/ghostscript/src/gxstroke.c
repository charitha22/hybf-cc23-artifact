...
if ( gs_debug_c('o') )
   {	int count = pgs_lp->dash.pattern_size;
	int i;
	gdprintf3("[o]half_width=%f, cap=%d, join=%d,\n",
		 pgs_lp->half_width, (int)pgs_lp->cap, (int)pgs_lp->join);
	gdprintf2("   miter_limit=%f, miter_check=%f,\n",
		 pgs_lp->miter_limit, pgs_lp->miter_check);
	gdprintf1("   dash pattern=%d", count);
	for ( i = 0; i < count; i++ )
	  gdprintf1(",%f", pgs_lp->dash.pattern[i]);
...
if ( gs_debug_c('O') )
   {	gdprintf4("[o]Intersect %f,%f(%f/%f)",
	...
	gdprintf3("\txdiff=%f ydiff=%f denom=%f ->\n",
		 xdiff, ydiff, denom);
   }
#endif
...
#ifdef DEBUG
if ( gs_debug_c('O') )
                   {	gdprintf4("[o]Miter check: u1/v1=%f/%f, u2/v2=%f/%f,\n",
				 u1, v1, u2, v2);
			gdprintf3("        num=%f, denom=%f, check=%f\n",
				 num, denom, check);
                   }
#endif
		    /*
