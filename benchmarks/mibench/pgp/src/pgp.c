...
char *pubring_file = NULL;
char *secring_file = NULL;
...
phone +1 303 541-0140\n"));
	...
	recipient = ParseRecipients(recipient);
	    ...
	    inputfile = myArgv[2];
	    pubring_file = myArgv[3];
	    secring_file = myArgv[4];
       printf("Here: %s; %s;\n", pubring_file, secring_file);
       strcpy(globalPubringName,pubring_file);
       strcpy(globalSecringName,secring_file);
	} else if (myArgc==6) {
	if (filter_mode) {
	    recipient = &myArgv[2];
	} else {
...
{
    printf("Here: do_decrypt\n");
    printf("globalPubringName: %s\n",globalPubringName);
    printf("globalSecringName: %s\n",globalSecringName);
    char *outfile = NULL;
    int status, i;
...
LANG("\nFile contains key(s).  Contents follow..."));
	    ...
	    if (ctb == CTB_CERT_SECKEY){
         ...
         /*
         if (secring_file) {
            strcpy(ringfile, secring_file);
            printf("Here: sec ring: %s\n", ringfile);
         } else {
         */
         ...
         printf("Here pub!\n");
         if (pubring_file) {
            strcpy(ringfile, pubring_file);
            printf("Here: pub ring: %s\n", ringfile);
         } else {
         */
