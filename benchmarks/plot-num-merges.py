...
for k in data.keys():
  ...
  for name in gorder:
    ...
    val = data[k][name]
    pdata[k][mapname[name]] = val
    #print(k, name, pdata[k][name])

plts.bars(pdata,'Profitable Fusions',groups=[mapname[n] for n in gorder],palette =colors,edgecolor='black',labelAverage=True,decimals=1,legendPosition='upper left')#,filename=path+'/code-size-reduction.pdf')
